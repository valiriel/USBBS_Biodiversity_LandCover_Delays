...
summary.m.iter <- as_tibble(results.q0.iter_2008$BUGSoutput$summary, rownames=NA) %>% round(5) %>% rownames_to_column %>% slice(-(50):-(n()-1)) 
summary.m.iter
results <- results.q0.iter_2008
attach(data)
source("USBBS_Modelling_Scripts/one more time/19_2008_model_fun.R")
detach(data)

data$delta.eq <- data$q0.eq - data$q0.now 

save(data, file=("data.2008.pred.rda"))

summary(data$q0.now)
summary(data$q0.eq)
summary(data$delta.eq)
...
scatter.q0 <- ggplot(data, aes(x=q0.t08, y = q0.now)) + 
...
ggsave(plot = scatter.q0, file = "USBBS_Modelling_Output/sr~vars_splines/2008_splines/srobs~srpred_spline.svg",
       units = "cm", dpi = "retina", width =15 , height = 15)
ggsave(plot = scatter.q0, file = "USBBS_Modelling_Output/sr~vars_splines/2008_splines/srobs~srpred_spline.pdf",
       units = "cm", dpi = "retina", width =15 , height = 15)

cor.test(data$q0.t08, data$q0.now)
...
spline.urban <- ggplot(data, aes(x=urban.t08)) + 
  geom_point(aes(y=q0.t08), color='gray38', alpha=0.15)+
...
ggsave(plot = spline.urban, file = "USBBS_Modelling_Output/sr~vars_splines/2008_splines/sr~urban_spline.svg",
       units = "cm", dpi = "retina", width =15 , height = 15)
ggsave(plot = spline.urban, file = "USBBS_Modelling_Output/sr~vars_splines/2008_splines/sr~urban_spline.pdf",
       units = "cm", dpi = "retina", width =15 , height = 15)
...
spline.cropland <- ggplot(data, aes(x=cropland.t08)) + 
  geom_point(aes(y=q0.t08), color='darkgoldenrod2', alpha=0.15)+
  geom_smooth(aes(y=q0.eq),  colour='black', alpha=0.5, method="auto", fullrange=T) + 
  theme_bw() + 
  scale_x_continuous( limits = c(0, 100), breaks = seq(from=0, to=100, by=20)) +
  #scale_x_log10(limits = c(-5, 100), breaks = seq(from=0, to=100, by=10)) +
...
ggsave(plot = spline.cropland, file = "USBBS_Modelling_Output/sr~vars_splines/2008_splines/sr~cropland_spline.svg",
       units = "cm", dpi = "retina", width =15 , height = 15)
ggsave(plot = spline.cropland, file = "USBBS_Modelling_Output/sr~vars_splines/2008_splines/sr~cropland_spline.pdf",
       units = "cm", dpi = "retina", width =15 , height = 15)
...
spline.grassland <- ggplot(data, aes(x=grassland.t08)) + 
  geom_point(aes(y=q0.t08), color='limegreen', alpha=0.15)+
  geom_smooth(aes(y=q0.eq), colour='black', alpha=0.5, method="auto", fullrange=T) +
  theme_bw() + 
  scale_x_continuous( limits = c(0, 100), breaks = seq(from=0, to=100, by=20)) +
  #scale_x_log10(limits = c(-5, 100), breaks = seq(from=0, to=100, by=10)) +
...
ggsave(plot = spline.grassland, file = "USBBS_Modelling_Output/sr~vars_splines/2008_splines/sr~grassland_spline.svg",
       units = "cm", dpi = "retina", width =15 , height = 15)
ggsave(plot = spline.grassland, file = "USBBS_Modelling_Output/sr~vars_splines/2008_splines/sr~grassland_spline.pdf",
       units = "cm", dpi = "retina", width =15 , height = 15)
...
spline.forest <- ggplot(data, aes(x=forest.t08)) + 
  geom_point(aes(y=q0.t08), color='darkgreen', alpha=0.15)+
  geom_smooth(aes(y=q0.eq), colour='black', alpha=0.5, method="auto", fullrange=T) + 
  theme_bw() +  
  scale_x_continuous( limits = c(0, 100), breaks = seq(from=0, to=100, by=20)) +
  #scale_x_log10(limits = c(-5, 100), breaks = seq(from=0, to=100, by=10)) +
...
ggsave(plot = spline.forest, file = "USBBS_Modelling_Output/sr~vars_splines/2008_splines/sr~forest_spline.svg",
       units = "cm", dpi = "retina", width =15 , height = 15)
ggsave(plot = spline.forest, file = "USBBS_Modelling_Output/sr~vars_splines/2008_splines/sr~forest_spline.pdf",
       units = "cm", dpi = "retina", width =15 , height = 15)
...
spline.temp <- ggplot(data, aes(x=tmean.t08)) + 
  geom_point(aes(y=q0.t08), color='darkred', alpha=0.15)+
  geom_smooth(aes(y=q0.eq), colour='black', alpha=0.5, method="auto", fullrange=T) + 
  theme_bw() +  
  scale_x_continuous( limits = c(7.5, 30), breaks = seq(from=0, to=100, by=5)) +
  #scale_x_log10(limits = c(-5, 100), breaks = seq(from=0, to=100, by=10)) +
  scale_y_continuous( limits = c(0, 80), breaks = seq(from = 0, to= 80, by = 10)) + 
  labs (x = "Temperature (°C)", y = "Species richness", title = '') + 
  geom_rug(colour="black", alpha=0.05 )

ggsave(plot = spline.temp, file = "USBBS_Modelling_Output/sr~vars_splines/2008_splines/sr~temp_spline.svg",
       units = "cm", dpi = "retina", width =15 , height = 15)
ggsave(plot = spline.temp, file = "USBBS_Modelling_Output/sr~vars_splines/2008_splines/sr~temp_spline.pdf",
       units = "cm", dpi = "retina", width =15 , height = 15)
...
spline.wetland <- ggplot(data, aes(x=wetland.t08)) + 
  geom_point(aes(y=q0.t08), color='cornflowerblue', alpha=0.15)+
  geom_smooth(aes(y=q0.eq), colour='black', alpha=0.5, method="auto", fullrange=T) + 
  theme_bw() +  
  scale_x_continuous( limits = c(0, 100), breaks = seq(from=0, to=100, by=20)) +
  #scale_x_log10(limits = c(-5, 100), breaks = seq(from=0, to=100, by=10)) +
...
ggsave(plot = spline.wetland, file = "USBBS_Modelling_Output/sr~vars_splines/2008_splines/sr~wetland_spline.svg",
       units = "cm", dpi = "retina", width =15 , height = 15)
ggsave(plot = spline.wetland, file = "USBBS_Modelling_Output/sr~vars_splines/2008_splines/sr~wetland_spline.pdf",
       units = "cm", dpi = "retina", width =15 , height = 15)
...
spline.all <- ggplot(data, aes(y=q0.eq)) +
  geom_smooth(aes(x=wetland.t08), colour='cornflowerblue', alpha=0.2, method="auto", size=1.1, fullrange=T) + 
  geom_smooth(aes(x=grassland.t08), colour='limegreen', alpha=0.2, method="auto", size=1.1, fullrange=T) + 
  geom_smooth(aes(x=cropland.t08), colour='darkgoldenrod2', alpha=0.2, method="auto", size=1.1, fullrange=T) +
  geom_smooth(aes(x=forest.t08), colour='darkgreen', alpha=0.2, method="auto", size=1.1, fullrange=T) + 
  geom_smooth(aes(x=urban.t08),colour='grey38', alpha=0.2, method="auto", size=1.1, fullrange=T) + 
  theme_bw() +  
...
ggsave(plot = spline.all, file = "USBBS_Modelling_Output/sr~vars_splines/2008_splines/sr~all_spline.svg",
       units = "cm", dpi = "retina", width =15 , height = 15)
ggsave(plot = spline.all, file = "USBBS_Modelling_Output/sr~vars_splines/2008_splines/sr~all_spline.pdf",
       units = "cm", dpi = "retina", width =15 , height = 15)
...
ggsave(plot = spline.panel, file = "USBBS_Modelling_Output/sr~vars_splines/2008_splines/PANEL_sr~vars_spline.svg",
       units = "cm", dpi = "retina", width =29.7 , height = 21)
ggsave(plot = spline.panel, file = "USBBS_Modelling_Output/sr~vars_splines/2008_splines/PANEL_sr~vars_spline.pdf",
       units = "cm", dpi = "retina", width =29.7 , height = 21)
